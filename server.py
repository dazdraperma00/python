...
class TaskQueueServer:
    ...
    def _add(self, client, command):
        ...
        self.save_all()  # Сохарняем все при добавлении
    ...
    def _get(self, client, command):
        ...
        timer = Timer(current_task, task_id, queue_name, self)
    ...
    def _ack(self, client, command):
        ...
        self.save_all()  # Сохраняем все при подтвержении
...
class Timer(Thread):
    def __init__(self, task, task_id, queue_name, server):
    ...
    def run(self):
        tasks = self.server.tasks[self.queue_name]
        in_process = self.server.in_process[self.queue_name]

        while self.timeout:
            time.sleep(1)
            # Клиент прислал ACK, timer больше не нужен
            if self.stop:
                return
            # Сервер завершил работу, необходимо вернуть задание и сохранить все в файл
            if not threading.main_thread().isAlive():
                heappush(tasks, (task_id, task))
                self.server.save_all()
                return
            self.timeout -= 1

        # Время вышло, клиент не успел прислать ACK
        in_process.pop(self.task_id)
        heappush(tasks, (task_id, task))
        self.server.save_all()
